...
$connection = new AMQPStreamConnection('images_rabbitmq', 5672, 'guest', 'guest');
...
try {
    ...
    $url = "http://web_app:80".substr($final_name, 1);
    $array = array('name' => $custom_name, 'url' => $url);
    $data = json_encode($array);

    // queue part 
    if (!empty($data)) {
        $channel->exchange_declare('images', 'fanout', false, false, false);
        $msg = new AMQPMessage(
            $data,
            array('delivery_mode' => AMQPMessage::DELIVERY_MODE_PERSISTENT)
        );
        $channel->basic_publish($msg, 'images');
    }
